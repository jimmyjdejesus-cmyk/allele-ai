...
class ObservableEvolutionEngine(EvolutionEngine):
    ...
    def __init__(self, config: EvolutionConfig, component_id: Optional[str] = None):
        ...
        self.metrics_collector = self.observability_engine.register_evolution_engine(
            self.component_id
        )
        ...
        logger.info(
            f"Observable evolution engine initialized: {self.component_id}",
    ...
    async def evolve(
        ...
        # Use %-style logging to keep message lines short for linters
        logger.info(
            "Starting evolution: %d genomes, %d generations",
            len(population),
            generations or self.config.generations,
            correlation_id=correlation_id,
        )

        try:
            # Record initial population metrics
            await self._record_population_metrics(
                population, generation=0, is_initial=True
            )

            # Run evolution with observability
            best_genome = await super().evolve(
                population, fitness_function, generations
            )
            ...
            logger.error(
                f"Evolution failed: {e}", correlation_id=correlation_id, exc_info=True
            )
            raise
        finally:
    ...
    async def _record_population_metrics(
        self,
        population: List[ConversationalGenome],
        generation: int,
        ...
        fitness_scores = [
            g.fitness_score for g in population if hasattr(g, "fitness_score")
        ]
        ...
        if not is_initial:
            # Generation timing
            generation_time = time.time() - getattr(
        ...
        self.metrics_collector.record_custom_metric(
            name="generation_number", value=generation, unit="count"
        )

        self.metrics_collector.record_custom_metric(
            name="best_fitness", value=best_fitness, unit="score"
        )

        self.metrics_collector.record_custom_metric(
            name="average_fitness", value=avg_fitness, unit="score"
        )

        self.metrics_collector.record_custom_metric(
            name="fitness_range", value=best_fitness - min_fitness, unit="score"
        )

        self.metrics_collector.record_custom_metric(
            name="population_diversity", value=diversity, unit="score"
        )

        self.metrics_collector.record_custom_metric(
            name="population_size", value=len(population), unit="count"
        )

        # Resource metrics
        self.metrics_collector.record_resource_usage(
            memory_mb=current_memory, cpu_percent=cpu_percent
        )
        ...
        if generation > 0:
            ...
            if len(self._fitness_history) > 0:
                ...
                self.metrics_collector.record_custom_metric(
                    name="fitness_improvement", value=improvement, unit="score"
                )
            ...
            if len(self._fitness_history) >= 5:
                recent_improvements = []
                for i in range(
                    max(0, len(self._fitness_history) - 5), len(self._fitness_history)
                ):
                    if i > 0:
                        improvement = (
                            self._fitness_history[i] - self._fitness_history[i - 1]
                        )
                        recent_improvements.append(improvement)

                if recent_improvements:
                    convergence_rate = sum(recent_improvements) / len(
                        recent_improvements
                    )
                    self.metrics_collector.record_custom_metric(
        ...
        if generation % 10 == 0 or is_initial:
            # Keep log message lines short for style checks
            logger.info(
                "Generation %d: best=%.4f, avg=%.4f, diversity=%.4f",
                generation,
                best_fitness,
                avg_fitness,
                diversity,
    ...
    async def _record_evolution_summary(
        self, best_genome: ConversationalGenome
    ) -> None:
        ...
        avg_generation_time = (
            total_time / total_generations if total_generations > 0 else 0
        )

        # Performance summary metrics
        self.metrics_collector.record_custom_metric(
            name="total_generations", value=total_generations, unit="count"
        )

        self.metrics_collector.record_custom_metric(
            name="total_evolution_time", value=total_time, unit="seconds"
        )

        self.metrics_collector.record_custom_metric(
            name="average_generation_time", value=avg_generation_time, unit="seconds"
        )

        self.metrics_collector.record_custom_metric(
            name="peak_memory_usage", value=self._peak_memory, unit="MB"
        )
        ...
        if self._fitness_history:
            ...
            self.metrics_collector.record_custom_metric(
                name="total_fitness_improvement", value=total_improvement, unit="score"
            )

            self.metrics_collector.record_custom_metric(
                name="fitness_improvement_rate",
                value=(
                    total_improvement / total_generations
        ...
        if len(self._diversity_history) > 1:
            ...
            self.metrics_collector.record_custom_metric(
                name="diversity_change", value=diversity_change, unit="score"
            )
        ...
        if self._cpu_samples:
            ...
            self.metrics_collector.record_custom_metric(
                name="average_cpu_usage", value=avg_cpu, unit="percent"
            )

            self.metrics_collector.record_custom_metric(
                name="peak_cpu_usage", value=max_cpu, unit="percent"
            )

        logger.info(
            "Evolution completed: %d generations, final fitness: %.4f",
            total_generations,
    ...
    async def _cleanup_evolution_metrics(self) -> None:
        ...
        self.metrics_collector.record_custom_metric(
            name="final_memory_usage", value=final_memory, unit="MB"
        )

        # Update final component performance metrics
        if (
            hasattr(self, "metrics_collector")
            and self.metrics_collector.performance_metrics
        ):
            ...
            perf.diversity_score = (
                self._diversity_history[-1] if self._diversity_history else 0
            )
...
class ObservableKrakenLNN(KrakenLNN):
    ...
    def __init__(
        ...
        # Initialize observability
        self.observability_engine = get_observability_engine()
        self.metrics_collector = self.observability_engine.register_kraken_lnn(
            self.component_id
        )
        ...
        logger.info(
            f"Observable Kraken LNN initialized: {self.component_id}",
    ...
    async def process_sequence(
        ...
        try:
            logger.debug(
                f"Processing sequence: {len(input_sequence)} inputs",
                correlation_id=correlation_id,
            )
            ...
            result = await super().process_sequence(
                input_sequence, learning_enabled, memory_consolidation
            )

            # Record processing metrics
            processing_time = time.time() - start_time
            await self._record_processing_metrics(
                processing_time, input_sequence, result
            )
            ...
            self.metrics_collector.record_latency(
                processing_time * 1000, "sequence_processing"
            )
            self.metrics_collector.record_success(False, "sequence_processing")

            logger.error(
                f"Sequence processing failed: {e}",
                correlation_id=correlation_id,
                exc_info=True,
            )
            raise
    ...
    async def _record_pre_processing_metrics(self, input_sequence: List[float]) -> None:
        ...
        self.metrics_collector.record_custom_metric(
            name="input_sequence_length", value=len(input_sequence), unit="count"
        )
        ...
        self.metrics_collector.record_custom_metric(
            name="memory_utilization", value=memory_utilization, unit="ratio"
        )

        # Reservoir state metrics
        reservoir_activity = float(self.liquid_reservoir.state.sum())
        self.metrics_collector.record_custom_metric(
            name="reservoir_activity", value=reservoir_activity, unit="sum"
        )
    ...
    async def _record_processing_metrics(
        ...
        self.metrics_collector.record_success(
            result.get("success", False), "sequence_processing"
        )
        ...
        if result.get("success"):
            ...
            self.metrics_collector.record_custom_metric(
                name="output_sequence_length", value=len(liquid_outputs), unit="count"
            )

            # Memory metrics
            memory_entries = result.get("memory_entries", 0)
            self.metrics_collector.record_custom_metric(
                name="memory_entries", value=memory_entries, unit="count"
            )
            ...
            if dynamics:
                for param, value in dynamics.items():
                    self.metrics_collector.record_custom_metric(
                        name=f"dynamics_{param}", value=float(value), unit="value"
                    )
            ...
            if hasattr(self.liquid_reservoir, "adaptive_weights"):
                ...
                for stat_name, stat_value in weight_stats.items():
                    self.metrics_collector.record_custom_metric(
                        name=f"weight_{stat_name}", value=stat_value, unit="value"
                    )
        ...
        self.metrics_collector.record_resource_usage(
            memory_mb=final_memory, cpu_percent=final_cpu
        )

        # Performance metrics
        throughput = len(input_sequence) / processing_time if processing_time > 0 else 0
        self.metrics_collector.record_custom_metric(
            name="processing_throughput", value=throughput, unit="inputs_per_second"
        )
        ...
        if self.metrics_collector.performance_metrics:
            ...
            perf.reservoir_utilization = (
                self._memory_utilization_history[-1]
                if self._memory_utilization_history
                else 0
            )
            perf.learning_rate = self._estimate_learning_rate()

        logger.debug(
            f"Sequence processed: {processing_time:.3f}s, {len(input_sequence)} inputs",
            correlation_id=self.metrics_collector.performance_metrics_correlation_id,
        )
    ...
    def _estimate_learning_rate(self) -> float:
        ...
        if len(recent_times) >= 2:
            time_variance = sum(
                (t - sum(recent_times) / len(recent_times)) ** 2 for t in recent_times
            ) / len(recent_times)
    ...
    async def get_network_state(self) -> Dict[str, Any]:
        ...
        state["observability"] = {
            ...
            "average_processing_time": (
                sum(self._sequence_times) / len(self._sequence_times)
                if self._sequence_times
                else 0
            ),
            "memory_utilization_history": self._memory_utilization_history[
                -10:
            ],  # Last 10
        }

        correlation_id = self.metrics_collector.performance_metrics_correlation_id
        state["observability"]["current_correlation_id"] = correlation_id
...
class ObservableNLPAgent(NLPAgent):
    ...
    def __init__(
        ...
        # Initialize observability
        self.observability_engine = get_observability_engine()
        self.metrics_collector = self.observability_engine.register_nlp_agent(
            self.component_id
        )
        ...
        logger.info(
            f"Observable NLP agent initialized: {self.component_id}",
    ...
    async def chat(
        ...
        try:
            logger.debug(
                f"Processing chat message: {len(message)} characters",
                correlation_id=correlation_id,
            )
            ...
            logger.error(
                f"Chat failed: {e}", correlation_id=correlation_id, exc_info=True
            )
            raise

    async def _record_pre_chat_metrics(
        self, message: str, context: Optional[Dict[str, Any]] = None
    ) -> None:
        ...
        # Message metrics
        self.metrics_collector.record_custom_metric(
            name="input_message_length", value=len(message), unit="characters"
        )

        self.metrics_collector.record_custom_metric(
            name="input_word_count", value=len(message.split()), unit="words"
        )
        ...
        if context:
            self.metrics_collector.record_custom_metric(
                name="has_context", value=1, unit="boolean"
            )
    ...
    async def _record_post_chat_metrics(
        self,
        processing_time: float,
        message: str,
        ...
        if hasattr(self, "conversation_buffer") and self.conversation_buffer:
            ...
            if last_turn.agent_response:
                ...
                self.metrics_collector.record_custom_metric(
                    name="response_length", value=response_length, unit="characters"
                )
                ...
                self.metrics_collector.record_custom_metric(
                    name="response_ratio", value=ratio, unit="ratio"
                )
        ...
        if self.llm_client and hasattr(self.llm_client, "metrics"):
            ...
            self.metrics_collector.record_custom_metric(
                name="llm_tokens_used", value=llm_metrics.total_tokens, unit="tokens"
            )

            self.metrics_collector.record_custom_metric(
                name="llm_cost", value=llm_metrics.total_cost, unit="USD"
            )
        ...
        self.metrics_collector.record_custom_metric(
            name="chat_throughput", value=throughput, unit="chats_per_second"
        )
        ...
        if self.metrics_collector.performance_metrics:
            ...
            if self.llm_client and hasattr(self.llm_client, "metrics"):
                perf.token_usage = {
                    ...
                    "completion": getattr(
                        self.llm_client.metrics, "completion_tokens", 0
                    ),
                }
        ...
        logger.debug(
            f"Chat completed: {processing_time:.3f}s",
            correlation_id=self.metrics_collector.performance_metrics_correlation_id,
        )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        base_metrics["observability"] = {
            ...
            "average_chat_time": (
                sum(self._conversation_times) / len(self._conversation_times)
                if self._conversation_times
                else 0
            ),
        ...
        correlation_id = self.metrics_collector.performance_metrics_correlation_id
        base_metrics["current_correlation_id"] = correlation_id
...
def create_observable_evolution_engine(
    config: EvolutionConfig,
) -> ObservableEvolutionEngine:
...
def create_observable_kraken_lnn(
    reservoir_size: int = 100,
    connectivity: float = 0.1,
    memory_buffer_size: int = 1000,
    dynamics: Optional[LiquidDynamics] = None,
    ...
    """
    return ObservableKrakenLNN(
        reservoir_size, connectivity, memory_buffer_size, dynamics
    )


def create_observable_agent(
    genome: ConversationalGenome, config: AgentConfig
) -> ObservableNLPAgent:
