...
def build_system_prompt(traits: Optional[Dict[str, float]]) -> str:
    ...
    if traits.get("empathy", 0.5) > 0.7:
        trait_descriptions.append(
            "Show deep understanding and emotional intelligence"
        )
    ...
    if traits.get("conciseness", 0.5) > 0.7:
        trait_descriptions.append(
            "Be direct and concise - give short, precise answers"
        )
    if traits.get("context_awareness", 0.5) > 0.7:
        trait_descriptions.append(
            "Maintain strong awareness of context and implications"
        )
    if traits.get("adaptability", 0.5) > 0.7:
        trait_descriptions.append(
            "Adapt communication style to the task requirements"
        )
    ...
    prompt += (
        ...
        "For math: show work then final number."
    )
    return prompt
...
def validate_trait_values(traits: Dict[str, float]) -> bool:
    ...
    for trait, value in traits.items():
        ...
        if not isinstance(value, (int, float)) or not (0.0 <= value <= 1.0):
            raise ValueError(
                f"Trait {trait} must be a number between 0.0 and 1.0, got {value}"
            )
...
"""
    ...
    for result in results:
        ...
        md += (
            f"| **{benchmark_name}** | {baseline:.1f}% | {phylogenic:.1f}% | {delta:+.1f}% | {status} |\n"
        )
